...
if st.session_state.etapa == 'chat':
    ...
    with st.chat_message("assistant"):
        ...
        st.write("Soy ConectIA, tu asistente personal. ¿En qué puedo ayudarte hoy?")
    ...
    if prompt := st.chat_input("Escribe aquí tu pregunta..."):
        ...
        with st.chat_message("assistant"):
            ...
            except: st.error("Lo siento, tuve un problema al procesar tu duda.")
...
# --- 7. PANTALLA: FORMULARIO ---
    ...
    with tab_api:
        ...
        if c_btn.button("Buscar Vuelo"):
            ...
            else: st.error("No pude encontrar ese vuelo.")
    ...
    with tab_manual:
        ...
        if st.button("🚀 CALCULAR TIEMPO", use_container_width=True): disparar_prediccion = True

    # --- 8. PROCESAMIENTO ---
    if disparar_prediccion:
        if origen == destino: st.error("❌ Origen y destino idénticos.")
        elif modelo_reg:
            with st.spinner('Analizando condiciones climatológicas...'):
                ...
                # ORDEN ESTRICTO SEGÚN GRÁFICA DE IMPORTANCIA
                input_dict = {
    ...
    if 'resultado_final' in st.session_state:
        ...
        if res['minutos'] > 45: c2.error(f"**Atención:** Se estima un retraso importante en {res['ruta']}.")
        elif res['minutos'] > 15: c2.warning(f"**Aviso:** Podrías tener una demora leve en {res['ruta']}.")
        else: c2.success(f"**Puntualidad:** Todo indica que tu vuelo en {res['ruta']} será puntual.")
